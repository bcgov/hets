...
gulp.task("js:modules", () => {
  return gulp
    .src(_.last(WEBPACK_CONFIG.entry.app))
    .pipe(devOnlyPlumber())
    .pipe(
      webpackStream(WEBPACK_CONFIG, webpack).on("error", (err) => {
...
gulp.task("test:unit", function () {
  return gulp.src(JS_UNIT_TEST_GLOB, { read: false }).pipe(
    $.mocha({
...
gulp.task(
  "assets:static",
  gulp.parallel("robots.txt", "images", "fonts", "css:vendor")
);
gulp.task("styles", gulp.parallel("sass", "css:vendor"));

gulp.task(
  "build:assets:simple",
  gulp.parallel("styles", "assets:static", "templates")
);
...
gulp.task(
  "default",
  gulp.series(IS_PRODUCTION ? "build:complete" : "build:dev")
);
