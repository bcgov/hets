...
import { Button, Row, Col, FormGroup, ControlLabel, HelpBlock, Checkbox } from 'react-bootstrap';
...
var EquipmentTransferDialog = React.createClass({
  ...
  validateSelectOwner() {
    ...
    if (this.state.donorOwnerCode === this.state.recipientOwnerCode) {
      this.setState({ recipientOwnerCodeError: 'The owner codes must be different' });
      return false;
    }
    ...
    if (this.state.seniorityOption === OPTION_EQUIPMENT_AND_SENIORITY.id && donor.localAreaId !== recipient.localAreaId) {
      this.setState({ seniorityOptionError: 'Both Transfer From and Transfer To Owners/Companies should be from the same local area' });
      return false;
    }
  ...
  renderSelectOwner() {
    ...
    return <div id="select-owner">
      <Row>
        <Col xs={4}>
          <ControlLabel>Transfer Equipment From (Owner Code) <sup>*</sup></ControlLabel>
        </Col>
        <Col xs={4}>
          <ControlLabel>Action <sup>*</sup></ControlLabel>
        </Col>
        <Col xs={4}>
          <ControlLabel>Transfer Equipment To (Owner Code) <sup>*</sup></ControlLabel>
        </Col>
      </Row>
      <Row>
        <Col xs={4}>
        ...
        <Col xs={4}>
          <FormGroup controlId="seniorityOption" validationState={ this.state.seniorityOptionError ? 'error' : null }>
            <DropdownControl id="seniorityOption" updateState={ this.updateState } selectedId={ this.state.seniorityOption } items={ seniorityOptions } />
            <HelpBlock>{ this.state.seniorityOptionError }</HelpBlock>
          </FormGroup>
        </Col>
        <Col xs={4}>
          <FormGroup controlId="recipientOwnerCode" validationState={ this.state.recipientOwnerCodeError ? 'error' : null }>
            <FormInputControl type="text" defaultValue={ this.state.recipientOwnerCode } updateState={ this.updateState } />
    ...
    return <TableControl id="select-equipment" headers={ headers }>
      {
        _.map(equipmentList, (equipment) => {
          return (
            <tr key={ equipment.id }>
              <td><Checkbox checked={ _.includes(this.state.selectedEquipmentIds, equipment.id) } onChange={ e => this.handleEquipmentCheckedChange(e, equipment.id) } /></td>
  ...
  render() {
    ...
    switch(this.state.stage) {
      ...
      case STAGE_CONFIRM:
        ...
        transferText = this.state.seniorityOption === OPTION_EQUIPMENT_AND_SENIORITY.id ? 'Transfer Equipment and Seniority' : 'Transfer Equipment Only';
